...
#include "wam_msgs/RTJointTraj.h"

//#include "control_msgs/FollowJointTrajectoryFeedback.h" // to publish to /feedback_states
#include "trajectory_msgs/JointTrajectory.h" // to subscribe to /joint_path_command
...
math::Vector<3>::type toRPY(Eigen::Quaterniond inquat)
{
  ...
  tf::Matrix3x3(q).getEulerYPR(newRPY[2], newRPY[1], newRPY[0]);
  return newRPY;
}
...
template<size_t DOF>
  ...
  public:
    WamNode(systems::Wam<DOF>& wam_) :
        wam(wam_), hand(NULL), ramp(NULL, SPEED), my_timer(NULL, SPEED)
    {
    }
    void
	...
	//Addition:
    void traj_end_CB (const trajectory_msgs::JointTrajectory::ConstPtr& msg);
    void joint_trajectory_sub_cb (wam_msgs::RTJointTraj);
...
template<size_t DOF>
  void WamNode<DOF>::init(ProductManager& pm)
  {
    ...
    pm.getExecutionManager()->startManaging(my_timer); //starting ramp manager
    ...
    wam.gravityCompensate(); // Turning on Gravity Compenstation by Default when starting the WAM Node
    ...
    //--------Addition-----------------------------------------------------------------------------------------
    traj_end = n_.subscribe("/joint_path_command", 1, &WamNode::traj_end_CB, this);
    joint_trajectory_sub = n_.subscribe("jnt_pos_traj",1,&WamNode::joint_trajectory_sub_cb, this);
   ...
   template<size_t DOF>
   void WamNode<DOF>::joint_trajectory_sub_cb (wam_msgs::RTJointTraj msg)
   {
      int vec_size = (int) msg.size;
      double jp1[vec_size];
      double jp2[vec_size];
      double jp3[vec_size];
      double jp4[vec_size];
      ...
      JpTraj<DOF> jp_trj(jp1, jp2, jp3, jp4, jp5, jp6, jp7, ts, vec_size, freq);

      systems::forceConnect(my_timer.output, jp_trj.input);
      wam.trackReferenceSignal(jp_trj.output);
      my_timer.smoothStart(0.5);

      while(!jp_trj.trajectoryIsComplete()){}
      systems::disconnect(jp_trj.input);
      my_timer.smoothStop(0.5);

   }

   template<size_t DOF>
   void WamNode<DOF>::traj_end_CB (const trajectory_msgs::JointTrajectory::ConstPtr& msg)
   {
   		std::vector<trajectory_msgs::JointTrajectoryPoint> traj_points = msg->points;
   		std::vector<int>::size_type num_points = traj_points.size();
   		jp_type joint_command;

   		joint_command[0] = traj_points[num_points-1].positions[0];
   		joint_command[1] = traj_points[num_points-1].positions[3];
   		joint_command[2] = traj_points[num_points-1].positions[4];
   		joint_command[3] = traj_points[num_points-1].positions[1];
   		joint_command[4] = traj_points[num_points-1].positions[6];
   		joint_command[5] = traj_points[num_points-1].positions[5];
   		joint_command[6] = traj_points[num_points-1].positions[2];

   		wam.moveTo(joint_command,true);
...
template<size_t DOF>
  void WamNode<DOF>::publishWam(ProductManager& pm)
  {
    ...
    // for (size_t i = 0; i < DOF; i++)
    // {
    //   joint_feedback_wam.actual.positions[i] = jp[i];
    //   joint_feedback_wam.actual.velocities[i] = jv[i];
    //   plan_scene.robot_state.joint_state.position[i] = jp[i];
    //   plan_scene.robot_state.joint_state.velocity[i] = jv[i];
    //   plan_scene.robot_state.joint_state.effort[i] = jt[i];

    // }

    // joint_feedback_wam.header.stamp = ros::Time::now();
    // joint_feedback_wam_pub.publish(joint_feedback_wam);

    // plan_scene.robot_state.joint_state.header.stamp = ros::Time::now();
    // plan_scene_pub.publish(plan_scene);
    //----------------------------------------------------------------------------------------------------
